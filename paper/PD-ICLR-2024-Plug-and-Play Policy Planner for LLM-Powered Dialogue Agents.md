# PD-ICLR-2024-Plug-and-Play Policy Planner for LLM-Powered Dialogue Agents
...
## 小记
 文章的两个核心约束是尽可能短的轮数增加成功率，但实际上尽可能短是否是合理的约束这一点仍然是一个问题，情感支持对话实际上就是一个很好的例子，短对话是否能够建立用户的较高信任感这有待考证。这么说的话本质上还是做了限制下的目标优化，后面是否能把这个变量去掉呢？这样的话训练周期又因为长对话上去了可能。另外LLM自博弈这个事情其实缺乏真实分布的支持，这个是我们做CogWM的一个优势，后面需要考虑的点是如何去把部分数据做分布泛化，做到更多场景。

 单条轨迹采样在因果意义上是不完备的，样本效率通常不足。它可以作为一种基础训练方式，但如果要追求更稳定、更高效、更能捕捉动作因果效应的策略，就需要引入树状采样、反事实推理或 off-policy 方法。